...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  const handleWhatsApp = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    const message = encodeURIComponent(`Merhaba, ${product.brand} ${product.name} hakkında bilgi almak istiyorum.`);
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        <h3 className="font-body text-sm font-medium text-foreground mb-3 line-clamp-2">
          {product.name}
        </h3>
        <button
          onClick={handleWhatsApp}
          className="w-full flex items-center justify-center gap-2 bg-[hsl(142,70%,40%)] hover:bg-[hsl(142,70%,35%)] text-white py-2.5 rounded text-sm font-body font-medium tracking-wider transition-colors"
        >
